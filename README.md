# ⚡ 智能充电桩调度计费系统 —— 开发进度与全栈收尾指南

## 🟢 一、 原始开发分工与最新完成情况

### 1. 数据模型与计费模块 —— 【已完成  组员c工作改为前端 见后文】
...
* **具体落实动作**：
    ...
    * `admin.py`（将上述三个类注册至 Django Admin，确保能通过后台修改充电桩状态模拟突发故障场景） —— **【🟢 100% 已完成】**
...
* **具体落实动作**：
    ...
    * **组员 E**：编写 `Query_PileState()` 收集 5 个充电桩运行指标；编写 `Query_QueueState()` 获取专属队列车辆列表。 —— **【🔴 暂未完成】**
...
* **极简网页开发 (`client.html`) —— 组员 C 主导**：
    * **页面布局**：纯静态 HTML 页面。上半部分为原生表单（输入框：输入车牌号；下拉框：选择快慢充模式、输入目标电量；按钮：【提交充电请求】、【中途修改】、【取消充电】）。下半部分放几行原生文本标签用于显示状态。
    * **核心逻辑**：使用 `setInterval` 开启一个 **每 2 秒** 的定时器，自动执行 `axios.get` 请求后端的 `Query_Charging_State` 接口。拿到 JSON 数据后，直接用 JavaScript 动态刷新文本内容，实现已充电量、当前账单金额在网页上的数据跳动。充电结束后弹窗渲染最终费用详单。
...
* **极简网页开发 (`admin_dashboard.html`) —— 组员 E 主导**：
    * **页面布局**：纯静态 HTML 页面。坚决不画图表，直接堆 2 个基础 HTML 表格（`<table>`）：
...
* 在全组项目整体拉通后，由 **组员 E** 负责主撰、**组员 B** 协助，将系统的数据库设计字段表、分时计费公式、中央调度状态机切换图等技术成果进行汇总，整合成最终交付的 Word 技术报告。

---

## 📅 三、 协作开发工作流纪律

1. **分支代码强制同步**：鉴于底层数据库模型与核心调度文件已更新推送，所有组员在开始各自的残余模块编写前，**必须先在终端运行 `git pull`**，随后**必须执行 `python manage.py migrate`** 同步本地数据库，以杜绝因模型不一致引发的运行时异常。
2. **跨域联调设置**：前端同学编写本地 `.html` 静态页面时，Axios 的请求基地址直接指向组长运行后端服务器的局域网 IP。后端已完成跨域资源共享（CORS）配置，联调接口无阻碍。
