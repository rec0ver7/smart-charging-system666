...
def auto_tick_piles():
    ...
    from charging_system.services.dispatch_service import (
        time_slice_schedule, priority_schedule, time_order_schedule
    )
    ...
    try:
        with transaction.atomic():
            ...
            # 尝试恢复 FAULT_WAITING 车辆（桩释放或 powerOn 恢复后）
            ...
            # 尝试将 WAITING 车辆调入空位
            waiting_exists = CarState.objects.filter(status='WAITING').exists()
            if waiting_exists:
                time_order_schedule()
    except Exception:
        pass  # tick 失败不阻塞查询
